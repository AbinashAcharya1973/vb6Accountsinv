...
Begin VB.Form frmdamagereturn 
   BorderStyle     =   4  'Fixed ToolWindow
   Caption         =   "Purchase Return [ Debit Note][Damage]"
   ClientHeight    =   9330
   ClientLeft      =   45
   ClientTop       =   210
   ClientWidth     =   14055
   ...
   Begin MSDBGrid.DBGrid DBGrid1 
      Bindings        =   "frmdamagereturn.frx":0000
      Height          =   3735
      Left            =   120
      OleObjectBlob   =   "frmdamagereturn.frx":0014
      TabIndex        =   73
      Top             =   3360
      Width           =   13815
   End
End
Attribute VB_Name = "frmdamagereturn"
...
Private Sub cboitemname_KeyDown(KeyCode As Integer, Shift As Integer)
    If KeyCode = 13 Then
        frmdamageproductlist.Show vbModal
    End If
    If KeyCode = 27 Then
        Me.TxtEtax.SetFocus
    End If
...
frmpurchasereturnprint.Show vbModal
End Sub
...
On Error GoTo errtrap
    ...
    If ans = 6 Then
        ...
        Set rec1 = db.OpenRecordset("select * from PurchaseReturnHead where Slno=" & Val(Me.txtslno.Text))
        If rec1.EOF Then
            AccId = 0
            db.Execute ("insert into PurchaseReturnHead (Slno,Purchasedate,InvNo,InvDate,Supplier,AccId,LrNo,TotalQty,TotalGross,ETax,ETaxAmount,CST,CSTAmount,VatAmount,NetAmount,RValue,GrandTotal,AmountInText,Freight,TotalMrp,RType) values(" & Me.txtslno.Text & ",'" & Me.txtStockindate.Text & "','" & Me.txtInvNo.Text & "','" & Me.txtInvDate.Text & "','" & Me.cboSupplier.Text & "'," & Me.cboSupplier.ItemData(Me.cboSupplier.ListIndex) & ",'" & Me.txtLrno.Text & "'," & Me.txtTotalqty.Text & "," & Me.txttotalgross.Text & "," & Me.TxtEtax.Text & "," & Me.txtetamount.Text & "," & Me.txtcst.Text & "," & Me.txtcstamount.Text & "," & Me.TxtVatAmount.Text & "," & Me.TxtNetAmount.Text & "," & Me.TxtRoundup.Text & "," & Me.txtGrandtotal.Text & ",'" & X & "'," & Val(Me.txtfreight.Text) & "," & Val(Me.txtmrpamount.Text) & ",'DAMAGE')")
        Else
            AccId = rec1("AccId")
            db.Execute ("update PurchaseReturnHead set Purchasedate='" & Me.txtStockindate.Text & "',InvNo='" & Me.txtInvNo.Text & "',InvDate='" & Me.txtInvDate.Text & "',Supplier='" & Me.cboSupplier.Text & "',AccId=" & Me.cboSupplier.ItemData(Me.cboSupplier.ListIndex) & ",LrNo='" & Me.txtLrno.Text & "',TotalQty=" & Me.txtTotalqty.Text & ",TotalGross=" & Me.txttotalgross.Text & ",ETax=" & Me.TxtEtax.Text & ",ETaxAmount=" & Me.txtetamount.Text & ",CST=" & Me.txtcst.Text & ",CSTAmount=" & Me.txtcstamount.Text & ",VatAmount=" & Me.TxtVatAmount.Text & ",NetAmount=" & Me.TxtNetAmount.Text & ",RValue=" & Me.TxtRoundup.Text & ",GrandTotal=" & Me.txtGrandtotal.Text & ",AmountInText='" & X & "',Freight=" & Val(Me.txtfreight.Text) & ",TotalMrp=" & Val(Me.txtmrpamount.Text) & " where Slno=" & Val(Me.txtslno.Text))
        End If

        'Check Existing Return (Update Stock)
        Set rec2 = db.OpenRecordset("select * from PurchaseReturnDetails where Slno=" & Val(Me.txtslno.Text))
        If Not rec2.EOF Then
            While Not rec2.EOF
                ...
                rec2.MoveNext
            Wend
            db.Execute ("delete * from PurchaseReturnDetails where SlNo=" & Val(Me.txtslno.Text))
        End If

        Set rec2 = db.OpenRecordset("select * from Temppurchasereturn")
        If Not rec2.EOF Then
            While Not rec2.EOF
                db.Execute ("insert into PurchaseReturnDetails (Slno,ItemName,Units,Pack,Qty,MRP,PrRate,Amount,ProductCode,Vat,VatAmount,Net,Free_Qty,mfgdate,expdate,batchno) values(" & Val(Me.txtslno.Text) & ",'" & Replace(rec2("ItemName"), "'", "''") & "','" & rec2("Units") & "'," & rec2("Pack") & "," & rec2("Qty") & "," & rec2("MRP") & "," & rec2("PrRate") & "," & rec2("Amount") & "," & rec2("ProductCode") & "," & rec2("Vat") & "," & rec2("VatAmount") & "," & rec2("Net") & "," & rec2("Free_Qty") & ",'" & rec2("mfgdate") & "','" & rec2("expdate") & "','" & rec2("batchno") & "')")
                'db.Execute ("update stock set Qty=Qty - " & (rec2("Qty") + rec2("Free_Qty")) & " where ProductCode=" & rec2("ProductCode"))
                db.Execute ("update damagestock set Qty=Qty-" & (rec2("Qty") + rec2("Free_Qty")) & " where ProductCode=" & rec2("ProductCode") & " and mfgdate='" & rec2("mfgdate") & "' and expdate='" & rec2("expdate") & "' and batchno='" & rec2("batchno") & "'")
                rec2.MoveNext
            Wend
        ...
        Me.txtGrandtotal.Text = "0.00"
        PURCHASEDELETE = "N"
        Set rec3 = db.OpenRecordset("select max(Slno)  as max_no from PurchaseReturnHead")
...
On Error GoTo errtrap
    ...
    Set rec1 = db.OpenRecordset("select max(slno) as max_no from PurchaseReturnHead")
...
Private Sub txtSlno_KeyDown(KeyCode As Integer, Shift As Integer)
    If KeyCode = 13 Then
        db.Execute ("delete * from Temppurchasereturn")
        Set rec2 = db.OpenRecordset("select * from PurchaseReturnHead where Slno=" & Me.txtslno.Text & " AND RTYPE='DAMAGE'")
        If Not rec2.EOF Then
            ...
            End If
            Set rec1 = db.OpenRecordset("select * from PurchaseReturnDetails where SlNo=" & Me.txtslno.Text)
            If Not rec1.EOF Then
                While Not rec1.EOF
                    db.Execute ("insert into Temppurchasereturn (ItemName,Units,Pack,Qty,Mrp,PrRate,Amount,ProductCode,Vat,VatAmount,Net,Free_Qty,Discount,SpDiscount,Discount_amount,mfgdate,expdate,batchno) values('" & Replace(rec1("ItemName"), "'", "''") & "','" & rec1("Units") & "'," & rec1("Pack") & "," & rec1("Qty") & "," & rec1("Mrp") & "," & rec1("PrRate") & "," & rec1("Amount") & "," & rec1("ProductCode") & "," & rec1("Vat") & "," & rec1("VatAmount") & "," & rec1("Net") & "," & rec1("Free_Qty") & "," & rec1("Discount") & "," & rec1("SpDiscount") & "," & rec1("Discount_amount") & ",'" & rec1("mfgdate") & "','" & rec1("expdate") & "','" & rec1("batchno") & "')")
                    rec1.MoveNext
                Wend
                Me.txtslno.Locked = True
            End If
        ...
        Else
            MsgBox "Not Found/Not Damage Return", vbCritical
        End If
        Me.Data1.Refresh
        '//--------------deleteing purchase-------------
        If PURCHASE_RETURN_DELETE = "Y" Then
            ...
            If ans = 6 Then
                ...
                Set rec2 = db.OpenRecordset("select * from PurchaseReturnDetails where Slno=" & Me.txtslno.Text)
                If Not rec2.EOF Then
                    While Not rec2.EOF
                        ...
                        rec2.MoveNext
                    Wend
                    db.Execute ("delete * from PurchaseReturnDetails where SlNo=" & Me.txtslno.Text)
                End If
                db.Execute ("delete * from PurchaseReturnHead where Slno=" & Me.txtslno.Text)
            End If
            db.Execute ("delete * from Temppurchasereturn")
            Data1.Refresh
            AccId = 0
