...
Begin VB.Form frmInvoiceLand 
   BorderStyle     =   4  'Fixed ToolWindow
   Caption         =   "Invoice Land"
   ClientHeight    =   9480
   ClientLeft      =   45
   ClientTop       =   210
   ClientWidth     =   14715
   ...
   Begin VB.Frame Frame3 
      ...
      Begin MSDBGrid.DBGrid DBGrid1 
         Bindings        =   "frmInvoiceLand.frx":0000
         Height          =   3855
         Left            =   120
         OleObjectBlob   =   "frmInvoiceLand.frx":0014
         TabIndex        =   58
         Top             =   0
         Width           =   14295
      End
   ...
   Begin VB.Frame Frame1 
      ...
      Begin VB.ComboBox cbocash 
         ...
         EndProperty
         Height          =   315
         ItemData        =   "frmInvoiceLand.frx":2C9F
         Left            =   7440
         List            =   "frmInvoiceLand.frx":2CA1
         Style           =   2  'Dropdown List
         TabIndex        =   0
         Top             =   240
         Width           =   1575
   ...
   End
End
Attribute VB_Name = "frmInvoiceLand"
...
If KeyCode = 13 Then
            ...
            Else
                 ...
                 Me.TxtRoundup.Text = Format(Round(Me.TxtNetAmount.Text) + Val(Me.txtfreight.Text) - Val(Me.TxtNetAmount.Text), "##0.00")
                 Me.txtGrandtotal.Text = Round(Val(Me.TxtNetAmount.Text) + Val(Me.txtfreight.Text))
                 db.Execute ("insert into Temp_Invoice (ItemName,ProductCode,Units,Qty,SaleRate,Gross,TradeDiscount,SpecialDiscount,DiscountAmount,MRP,Vat,VatAmount,Net,Free_Qty,Tax_type,expdate,slno,mfgdate,batchno,adapterslno,batteryslno) values('" & Replace(Me.cboitemname.Text, "'", "''") & "'," & Me.txtproductcode.Text & ",'" & Me.cbounit.Text & "'," & Val(Me.txtQty.Text) & "," & Val(Me.txtsalerate.Text) & "," & Me.txtgross.Text & "," & Me.txttradediscount.Text & "," & Me.txtspecialdiscount.Text & "," & TradeDiscount + SpecialDiscount & "," & Val(Me.txtmrp.Text) & "," & Me.TxtVat.Text & "," & Me.txttaxamount.Text & "," & Me.txtamount.Text & "," & Me.txtfree.Text & ",'" & Me.txttaxtype.Text & "','" & Me.txtpack.Text & "'," & temp_slno & ",'" & Me.txtmfgdate.Text & "','" & Me.cbobatch.Text & "','" & Me.txtadapter.Text & "','" & Me.txtbattery.Text & "')")
            End If
            Me.Data1.RecordSource = "select * from Temp_Invoice order by slno desc"
            Me.Data1.Refresh
...
'            Me.txtQty.Text = 0
'        End If
        txtsplit = Split(UCase(Me.txtpack.Text), "X")
        varL = txtsplit(0)
        varW = txtsplit(1)
        Me.txtQty.Text = Val(varL) * Val(varW)
    End If
    Me.txtQty.SetFocus
End If
End Sub
