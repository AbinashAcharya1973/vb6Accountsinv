...
Begin VB.Form frmStockin 
   ...
   Caption         =   "Stock Entry/Purchase Entry"
   ClientHeight    =   9225
   ClientLeft      =   45
   ClientTop       =   210
   ClientWidth     =   14340
   ...
   Begin MSDBGrid.DBGrid DBGrid1 
      Bindings        =   "frmStockin.frx":0000
      Height          =   3495
      Left            =   120
      OleObjectBlob   =   "frmStockin.frx":0014
      TabIndex        =   52
      Top             =   3000
      Width           =   14115
   End
...
Me.lblmessage.Caption = "Press Enter to goto Next Or Press F2 to Close"
End Sub
...
Me.lblmessage.Caption = "Press Enter to Search Products/F2 to Add New Item"
End Sub

Private Sub cboitemname_KeyDown(KeyCode As Integer, Shift As Integer)
    If KeyCode = 13 Then
        SEARCHWORD = Me.cboitemname.Text
        frmproductlist1.Show vbModal
    End If
    If KeyCode = 27 Then
        Me.txtcst.SetFocus
    End If
...
Private Sub cbomtype_LostFocus()
...
Private Sub CmdSave_Click()
    ...
    If ans = 6 Then
        ...
        End If
        Set rec = dbs.OpenRecordset("select * from Temp_Stockin")
        If Not rec.EOF Then
            temp_grandtotal = Me.txtGrandtotal.Text
            X = NumberToWord(Trim(str(Round(Val(temp_grandtotal)))))
            Set rec1 = dbs.OpenRecordset("select * from Purchasehead where Slno=" & Slno)
            If rec1.EOF Then
                AccId = 0
                dbs.Execute ("insert into PurchaseHead (Slno,Purchasedate,InvNo,InvDate,Supplier,AccId,LrNo,TotalQty,TotalGross,VatAmount,NetAmount,RValue,GrandTotal,AmountInText,Freight,TotalMrp,lessDiscount,Waybill,CST,CSTAmount,ETax,ETaxAmount) values(" & Slno & ",'" & Me.txtStockindate.Text & "','" & Me.txtinvno.Text & "','" & Me.txtinvdate.Text & "','" & Me.cboSupplier.Text & "'," & Me.cboSupplier.ItemData(Me.cboSupplier.ListIndex) & ",'" & Me.txtLrNo.Text & "'," & Me.txttotalqty.Text & "," & Me.txttotalgross.Text & "," & Me.TxtVatAmount.Text & "," & Me.TxtNetAmount.Text & "," & Me.TxtRoundup.Text & "," & Me.txtGrandtotal.Text & ",'" & X & "'," & Val(Me.txtfreight.Text) & "," & Val(Me.txtmrpamount.Text) & "," & Val(Me.txtdiscountamount.Text) & ",'" & Me.txtwaybill.Text & "'," & Me.txtcst.Text & "," & Me.txtcstamount.Text & "," & Me.txtet.Text & "," & Me.txtetamount.Text & ")")
            Else
                AccId = rec1("AccId")
                dbs.Execute ("update PurchaseHead set Purchasedate='" & Me.txtStockindate.Text & "',InvNo='" & Me.txtinvno.Text & "',InvDate='" & Me.txtinvdate.Text & "',Supplier='" & Me.cboSupplier.Text & "',AccId=" & Me.cboSupplier.ItemData(Me.cboSupplier.ListIndex) & ",LrNo='" & Me.txtLrNo.Text & "',TotalQty=" & Me.txttotalqty.Text & ",TotalGross=" & Me.txttotalgross.Text & ",VatAmount=" & Me.TxtVatAmount.Text & ",NetAmount=" & Me.TxtNetAmount.Text & ",RValue=" & Me.TxtRoundup.Text & ",GrandTotal=" & Me.txtGrandtotal.Text & ",AmountInText='" & X & "',Freight=" & Val(Me.txtfreight.Text) & ",TotalMrp=" & Val(Me.txtmrpamount.Text) & ",lessDiscount=" & Val(Me.txtdiscountamount.Text) & ",Waybill='" & Me.txtwaybill.Text & "',CST=" & Me.txtcst.Text & ",CSTAmount=" & Me.txtcstamount.Text & ",ETax=" & Me.txtet.Text & ",ETaxAmount=" & Me.txtetamount.Text & " where Slno=" & Slno)
            End If

            Set rec2 = dbs.OpenRecordset("select * from PurchaseDetails where Slno=" & Slno)
            If Not rec2.EOF Then
                While Not rec2.EOF
                    Set rec3 = dbs.OpenRecordset("select * from Stock where productcode=" & rec2("Productcode"))
                    If Not rec3.EOF Then
                        dbs.Execute ("update stock set Qty=Qty - " & (rec2("Qty") + rec2("Free_Qty")) & " where productcode=" & rec2("Productcode"))
                        dbs.Execute ("update stockdetails set Qty=Qty - " & (rec2("Qty") + rec2("Free_Qty")) & " where productcode=" & rec2("Productcode") & " and mfgdate='" & rec2("mfgdate") & "' and expdate='" & rec2("expdate") & "' and batchno='" & rec2("batchno") & "'")
                    End If
                    rec2.MoveNext
                Wend
                dbs.Execute ("delete * from PurchaseDetails where SlNo=" & Slno)
            End If

            'Set rec2 = db.OpenRecordset("select * from Temp_Stockin where Usname='" & usname & "'")
            Set rec2 = dbs.OpenRecordset("select * from Temp_Stockin")
            If Not rec2.EOF Then
                While Not rec2.EOF
                    dbs.Execute ("insert into PurchaseDetails (Slno,ItemName,Units,Pack,Qty,MRP,PrRate,Amount,ProductCode,Vat,VatAmount,Net,Free_Qty,Discount,SpDiscount,Discount_amount,cd,mfgdate,expdate,batchno) values(" & Slno & ",'" & Replace(rec2("ItemName"), "'", "''") & "','" & rec2("Units") & "'," & rec2("Pack") & "," & rec2("Qty") & "," & rec2("MRP") & "," & rec2("PrRate") & "," & rec2("Amount") & "," & rec2("ProductCode") & "," & rec2("Vat") & "," & rec2("VatAmount") & "," & rec2("Net") & "," & rec2("Free_Qty") & "," & rec2("Discount") & "," & rec2("SpDiscount") & "," & rec2("Discount_amount") & "," & rec2("cd") & ",'" & rec2("mfgdate") & "','" & rec2("expdate") & "','" & rec2("batchno") & "')")
                    Set rec3 = dbs.OpenRecordset("select * from Stock where ProductCode=" & rec2("ProductCode"))
                    If Not rec3.EOF Then
                        dbs.Execute ("update stock set Qty=Qty + " & (rec2("Qty") + rec2("Free_Qty")) & " where ProductCode=" & rec2("ProductCode"))
                    Else
                        dbs.Execute ("insert into Stock (itemname,MRP,PRate,Qty,ProductCode) values('" & Replace(rec2("itemname"), "'", "''") & "'," & rec2("MRP") & "," & rec2("PrRate") & "," & (rec2("Qty") + rec2("Free_Qty")) & "," & rec2("ProductCode") & ")")
                    End If
                    Set rec3 = dbs.OpenRecordset("select * from Stockdetails where productcode=" & rec2("Productcode") & " and mfgdate='" & rec2("mfgdate") & "' and expdate='" & rec2("expdate") & "' and batchno='" & rec2("batchno") & "'")
                    If Not rec3.EOF Then
                        dbs.Execute ("update stockdetails set Qty=Qty + " & (rec2("Qty") + rec2("Free_Qty")) & " where productcode=" & rec2("Productcode") & " and mfgdate='" & rec2("mfgdate") & "' and expdate='" & rec2("expdate") & "' and batchno='" & rec2("batchno") & "'")
                    Else
                        Set rec1 = dbs.OpenRecordset("select * from itemmaster where productcode=" & rec2("productcode"))
                        If Not rec1.EOF Then
                            dbs.Execute ("insert into Stockdetails (ProductType,itemtype,itemname,brand,barcode,size,MRP,PRate,Qty,ProductCode,Vat,Lose,UniyType,SaleRate,mfgdate,batchno,expdate,hsn,purchaseinfo) values('" & rec1("ProductType") & "','" & rec1("itemtype") & "','" & Replace(rec1("item"), "'", "''") & "','" & rec1("brand") & "','" & Replace(rec1("barcode"), "'", "''") & "','" & rec1("size") & "'," & rec1("mrp") & "," & rec1("purchaserate") & "," & rec2("qty") + rec2("free_qty") & "," & rec2("Productcode") & "," & rec2("vat") & "," & rec1("Lose") & ",'" & rec1("unittype") & "'," & rec1("SaleRate") & ",'" & rec2("mfgdate") & "','" & rec2("batchno") & "','" & rec2("expdate") & "','" & rec1("HSN") & "','" & Me.txtslno.Text & "-" & Me.txtStockindate.Text & "-" & rec1("productcode") & "')")
                        End If
                    End If
                    rec2.MoveNext
                Wend

                dbs.Execute ("delete * from LedgerTran Where VoucherType='Purchase' and VoucherSlno=" & Me.txtslno.Text)
                '--------Party Ledger New Entry--------------------------
                Set rec3 = dbs.OpenRecordset("select * from LedgerMaster where AccName like 'Purchase*'")
                If Not rec3.EOF Then
                    prAccId = rec3("AccId")
                End If
                Set rec3 = dbs.OpenRecordset("select * from LedgerTran where AccId=" & Me.cboSupplier.ItemData(Me.cboSupplier.ListIndex) & " and Slno=(select max(SlNo) from LedgerTran where AccId=" & Me.cboSupplier.ItemData(Me.cboSupplier.ListIndex) & ")")
                If Not rec3.EOF Then
                ...
                End If
                dbs.Execute ("insert into LedgerTran (Slno,TDate,Particulars,Dr,Cr,Balance,AccId,Remarks,VoucherType,VoucherSlno,GroupId,TranAccId) values (" & temp_ledger_slno & ",'" & Me.txtStockindate.Text & "','By Purchase',0," & temp_grandtotal & "," & temp_ledger_balance + temp_grandtotal & "," & Me.cboSupplier.ItemData(Me.cboSupplier.ListIndex) & ",'Inv No:" & Me.txtinvno.Text & "-" & Me.txtinvdate.Text & "','Purchase'," & Slno & "," & TEMP_GROUPID & "," & prAccId & ")")
                ...
                If Not rec3.EOF Then
                    ...
                    End If
                    dbs.Execute ("insert into LedgerTran (Slno,TDate,Particulars,Dr,Cr,Balance,AccId,Remarks,VoucherType,VoucherSlno,GroupId,TranAccId) values(" & TEMP_PURCHASE_LEDGER_SLNO & ",'" & Me.txtStockindate.Text & "','" & Me.cboSupplier.Text & "'," & temp_purchaseamount & ",0," & TEMP_PURCHASE_LEDGER_BALANCE + temp_purchaseamount & "," & rec3("AccId") & ",'Inv No:" & Me.txtinvno.Text & "-" & Me.txtinvdate.Text & "','Purchase'," & Slno & "," & rec3("GroupId") & "," & Me.cboSupplier.ItemData(Me.cboSupplier.ListIndex) & ")")
                End If
                    ...
                    If Not rec3.EOF Then
                        If Val(Me.TxtVatAmount.Text) > 0 Then
                            ...
                            Else
                                ...
                                TAX_LEDGER_BALANCE = 0
                            End If
                            dbs.Execute ("insert into LedgerTran (Slno,TDate,Particulars,Dr,Cr,Balance,AccId,Remarks,VoucherType,VoucherSlno,GroupId,TranAccId) values (" & TAX_LEDGER_SLNO & ",'" & Me.txtStockindate.Text & "','" & Me.cboSupplier.Text & "'," & Val(Me.TxtVatAmount.Text) / 2 & ",0," & TAX_LEDGER_BALANCE - (Val(Me.TxtVatAmount.Text) / 2) & "," & rec3("AccId") & ",'Inv No:" & Me.txtinvno.Text & "-" & Me.txtinvdate.Text & "','Purchase'," & Slno & "," & rec3("GroupId") & "," & Me.cboSupplier.ItemData(Me.cboSupplier.ListIndex) & ")")
                        End If
                    End If
                End If
            ...
            dbs.Execute ("delete * from Temp_Stockin")
            'Data1.Refresh
            AccId = 0
            Slno = 0
            prAccId = 0
            ...
            Me.txtGrandtotal.Text = "0.00"
            PURCHASEDELETE = "N"
            Set rec3 = dbs.OpenRecordset("select max(Slno)  as max_no from PurchaseHead")
...
Private Sub Form_Load()
    ...
    If Not IsNull(rec1!max_no) Then
        ...
        If SoftwareVersion = "Demo" And Val(Me.txtslno.Text) > 6 Then
...
Private Sub lstinv_Click()
    ...
    Me.txtinvdate.Text = Format(JSONRec(invid).Item("InvDate"), "dd/mm/yyyy")
...
If KeyCode = 13 Then
    Me.txtcstamount.Text = Round((Val(Me.txttotalgross.Text) - Val(Me.txtdiscountamount.Text) + Val(Me.TxtVatAmount.Text)) * (Val(Me.txtcst.Text) / 100), 2)
    Me.txtet.SetFocus
End If
End Sub
...
Private Sub txthsn1_GotFocus()
Me.txthsn1.SelStart = 0
Me.txthsn1.SelLength = Len(Me.txthsn1.Text)
End Sub

Private Sub txthsn1_KeyDown(KeyCode As Integer, Shift As Integer)
If KeyCode = 13 Then
    Set rec1 = db.OpenRecordset("select * from itemmaster where productcode=" & Me.txtproductcode.Text & " and hsn='" & Me.txthsn1.Text & "'")
    If rec1.EOF Then
        db.Execute "update itemmaster set hsn='" & Me.txthsn1.Text & "' where productcode=" & Me.txtproductcode.Text
        db.Execute "update stock set hsn='" & Me.txthsn1.Text & "' where productcode=" & Me.txtproductcode.Text
    End If
    Me.cbobatch.Text = Me.txtproductcode.Text & "-" & Me.txtStockindate.Text & "-" & Me.txtslno.Text
    'Me.txtmfgdate.Text = Format(Me.txtStockindate.Text, "mm") & "/" & Format(Me.txtStockindate.Text, "yyyy")
        ...
        If Not rec1.EOF Then
            If rec1("purchaserate") <> Val(Me.txtPrate.Text) Then
                ...
                If ans = 6 Then
                    ...
                    db.Execute ("Update stock set Prate=" & Me.txtPrate.Text & " where ProductCode=" & Val(Me.txtproductcode.Text))
                Else
                    Me.txtdiscount.SetFocus
                End If
            End If
...
Private Sub TxtRoundup_KeyDown(KeyCode As Integer, Shift As Integer)
    If KeyCode = 13 Then
        Me.txtGrandtotal.Text = Format(Round(Val(Me.TxtNetAmount.Text)), "#####0.00")
    ...
    If KeyCode = 13 Then
        ...
        Else
            ...
            If ans = 6 Then
                ...
                Me.cboitemname.Text = Me.txtiname.Text
        ...
        If Not rec1.EOF Then
            If rec1("tax") <> Val(Me.TxtVat.Text) Then
                ...
                If ans = 6 Then
                    ...
                    db.Execute ("Update stock set vat=" & Me.TxtVat.Text & " where ProductCode=" & Val(Me.txtproductcode.Text))
                Else
                   Me.txtdiscount.SetFocus
                End If
            End If
